package com.hampcode.apistream.filtermapflatmap.filter;
...
public class A_StreamFilter {
	public static void main(String[] args) {

		List<Persona> personas = Arrays.asList(new Persona("Ana", LocalDate.of(1930, 12, 20)),
				new Persona("Jesús", LocalDate.of(1931, 3, 13)), new Persona("Miguel", LocalDate.of(1943, 8, 1)),
				new Persona("Simón", LocalDate.of(1949, 5, 22)), new Persona("Luisa", LocalDate.of(1959, 8, 28)),
				new Persona("Antonio", LocalDate.of(1965, 4, 28)), new Persona("Alicia", LocalDate.of(1974, 2, 10)),
				new Persona("Ángel", LocalDate.of(1982, 10, 12)), new Persona("Alberto", LocalDate.of(2000, 1, 2)),
		...
		personas.stream().filter(p -> p.getEdad() >= 18).map(Persona::getNombre).forEach(System.out::println);
		System.out.println("");

		// Obtener todas las personas en edad activa
		System.out.println("PERSONAS EN EDAD ACTIVA (18-65 AÑOS)");
		personas.stream().filter(p -> p.getEdad() >= 18 && p.getEdad() <= 65)
		...
		long nPersonas = personas.stream().filter(p -> p.getFechaNacimiento().getYear() >= 2001).count();
		...
		Persona p1 = personas.stream().filter(p -> p.getNombre().equalsIgnoreCase("Andrés")).findAny()
				.orElse(new Persona());

		System.out.println(p1);

		Predicate<Persona> predicate = (Persona p) -> p.getNombre().toLowerCase().contains("s")
				&& (p.getEdad() >= 60 && p.getEdad() <= 90);

		Persona p2 = personas.stream().filter(predicate).findAny().orElse(new Persona());

		System.out.println(p2);

		personas.stream().filter(predicate).findFirst().ifPresent(System.out::println);
