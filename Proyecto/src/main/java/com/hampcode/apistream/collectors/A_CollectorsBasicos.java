package com.hampcode.apistream.collectors;
...
public class A_CollectorsBasicos {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * La clase Random incluye métodos que trabajan con streams, y nos permiten
		 * crear uno de números aleatorios (ints). A continuación, lo limitamos a 20
		 * elementos (limit). Posteriormente, indicamos que en lugar de un IntStream,
		 * queremos obtener un Stream<Integer> (boxed). Por último, usamos la operación
		 * collect con el primer Collector del ejemplo: toList, que nos devuelve un
		 * List<Integer> a partir del stream.
		 * 
		 */
		Random r = new Random();
